...
// .wrangler/tmp/bundle-43ga23/checked-fetch.js
...
// .wrangler/tmp/bundle-43ga23/strip-cf-connecting-ip-header.js
...
var AuthService = class {
  ...
  async initiateLogin(phoneNumber) {
    ...
    try {
      await this.whatsappService.sendInteractiveButtonMessage(
        ...
        buttonText,
        plainToken,
        // Use plain-text token as button payload
        bodyText
      );
      return { success: true };
    } catch (error) {
  ...
  async verifyWebhookToken(phoneNumber, plainToken) {
...
var VerificationService = class {
  ...
  async createVerificationToken(phoneNumber) {
    const now = Math.floor(Date.now() / 1e3);
  ...
  async validateAndConsumeToken(plainToken, phoneNumber) {
    const now = Math.floor(Date.now() / 1e3);
    const tokenHash = await hashToken(plainToken);
          ...
          if (change.field === "messages") {
            ...
            if (value.messages && value.messages.length > 0) {
              for (const message2 of value.messages) {
                ...
                if (message2.type === "button" && message2.button) {
                  const buttonPayload = message2.button.payload;
...
// .wrangler/tmp/bundle-43ga23/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-43ga23/middleware-loader.entry.ts
