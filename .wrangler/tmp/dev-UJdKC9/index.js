...
// .wrangler/tmp/bundle-PxxfFi/checked-fetch.js
...
// .wrangler/tmp/bundle-PxxfFi/strip-cf-connecting-ip-header.js
...
function initializeConfig(env) {
  ...
  CONFIG.MOCK.WHATSAPP_API = env.MOCK_WHATSAPP_API === "true";
  return CONFIG;
}
...
var WhatsAppService = class {
  ...
  async sendInteractiveButtonMessage(to, buttonText, buttonPayload, bodyText) {
    ...
    return this.sendMessage(to, message2);
  }
  async sendLoginLink(to, loginToken) {
    if (CONFIG.MOCK.WHATSAPP_API) {
  ...
  async sendHelloWorldMessage(to) {
    if (CONFIG.MOCK.WHATSAPP_API) {
...
async function hashToken(token) {
  ...
  const data = encoder3.encode(token);
...
var UserService = class {
  ...
  async findUserByPhone(phoneNumber) {
    const result = await this.db.prepare("SELECT * FROM users WHERE phone_number = ?").bind(phoneNumber).first();
    return result || null;
  }
  ...
  async createUser(phoneNumber, name = null) {
    ...
    await this.db.prepare("INSERT INTO users (id, phone_number, name, created_at, last_login) VALUES (?, ?, ?, ?, ?)").bind(userId, phoneNumber, name, now, now).run();
    return {
      id: userId,
  ...
  async updateLastLogin(userId) {
    const now = Math.floor(Date.now() / 1e3);
    await this.db.prepare("UPDATE users SET last_login = ? WHERE id = ?").bind(now, userId).run();
  }
  async createSession(userId, expiresIn) {
    const now = Math.floor(Date.now() / 1e3);
    const sessionId = generateRandomId(32);
    const expiresAt = now + expiresIn;
    await this.db.prepare("INSERT INTO sessions (id, user_id, created_at, expires_at, status) VALUES (?, ?, ?, ?, ?)").bind(sessionId, userId, now, expiresAt, "pending").run();
    return {
      id: sessionId,
      user_id: userId,
      created_at: now,
  ...
  async updateSessionTokens(sessionId, authToken, refreshToken) {
    await this.db.prepare("UPDATE sessions SET auth_token = ?, refresh_token = ?, status = ? WHERE id = ?").bind(authToken, refreshToken, "ready", sessionId).run();
  }
  async getSessionTokens(sessionId) {
    const session = await this.db.prepare("SELECT auth_token, refresh_token FROM sessions WHERE id = ? AND status = ?").bind(sessionId, "ready").first();
    if (session && session.auth_token && session.refresh_token) {
      await this.db.prepare("UPDATE sessions SET status = ? WHERE id = ?").bind("completed", sessionId).run();
      return { authToken: session.auth_token, refreshToken: session.refresh_token };
    }
    return null;
  }
  async findSessionById(sessionId) {
    const result = await this.db.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > ?").bind(sessionId, Math.floor(Date.now() / 1e3)).first();
    return result || null;
  }
  async findSessionByUserId(userId) {
    const result = await this.db.prepare("SELECT * FROM sessions WHERE user_id = ? AND expires_at > ? ORDER BY created_at DESC LIMIT 1").bind(userId, Math.floor(Date.now() / 1e3)).first();
    return result || null;
  }
...
async function createJWT(payload, expiresIn = CONFIG.JWT.EXPIRY) {
  const secret = encoder2.encode(CONFIG.JWT.SECRET);
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(Math.floor(Date.now() / 1e3) + expiresIn).sign(secret);
}
...
var AuthService = class {
  ...
  constructor(userService, whatsappService, verificationService) {
  ...
  async initiateLogin(phoneNumber) {
    ...
    const bodyText = isNewUser ? "Hello!\n\nWe've received a request to create a new account with this phone number.\n\nPlease press the button below in the next 10 minutes to confirm. If you haven't made this request, you can safely ignore this message." : "Welcome back!\n\nTap the button below to log in to your account. This link will expire in 10 minutes.";
    ...
    try {
      ...
      await this.whatsappService.sendInteractiveButtonMessage(
        ...
        buttonText,
        encodedToken,
        // Use encoded token as button payload
        bodyText
      );
  ...
  async verifyWebhookToken(phoneNumber, encodedToken) {
  ...
  async verifyLoginTokenOnly(token) {
...
var VerificationService = class {
  static {
    __name(this, "VerificationService");
  }
  constructor(db) {
    this.db = db;
  }
  async createVerificationToken(phoneNumber, isNewUser) {
    ...
    await this.db.prepare("INSERT INTO verification_tokens (id, token_hash, phone_number, expires_at, created_at) VALUES (?, ?, ?, ?, ?)").bind(tokenId, tokenHash, phoneNumber, expiresAt, now).run();
    console.log(`[VerificationService] Token saved to database successfully`);
    return { token: encodedPayload, tokenId };
  }
  async validateAndConsumeToken(encodedToken, phoneNumber) {
    ...
    const tokenRecord = await this.db.prepare("SELECT * FROM verification_tokens WHERE token_hash = ? AND phone_number = ? AND expires_at > ? AND used_at IS NULL").bind(tokenHash, phoneNumber, now).first();
    if (!tokenRecord) {
    ...
    await this.db.prepare("UPDATE verification_tokens SET used_at = ? WHERE id = ?").bind(now, tokenRecord.id).run();
    console.log(`[VerificationService] Token marked as used`);
    return { isValid: true, isNewUser: payload.isNewUser };
  }
  async decodeTokenPayload(encodedToken) {
    ...
    try {
      ...
      if (!decodedPayload.token || typeof decodedPayload.isNewUser !== "boolean" || !decodedPayload.phoneNumber || !decodedPayload.timestamp || !decodedPayload.expiresAt) {
  ...
  async getPhoneNumberFromToken(encodedToken) {
  ...
  async validateTokenOnly(encodedToken, phoneNumber) {
    const payload = await this.decodeTokenPayload(encodedToken);
    if (!payload) {
      return { isValid: false };
    }
  ...
  async createRefreshToken(userId) {
    ...
    await this.db.prepare("INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at, created_at) VALUES (?, ?, ?, ?, ?)").bind(tokenId, userId, tokenHash, expiresAt, now).run();
    return { token: plainToken, tokenId };
  }
  async validateRefreshToken(plainToken, userId) {
    const now = Math.floor(Date.now() / 1e3);
    const tokenHash = await hashToken(plainToken);
    const tokenRecord = await this.db.prepare("SELECT * FROM refresh_tokens WHERE token_hash = ? AND user_id = ? AND expires_at > ? AND revoked_at IS NULL").bind(tokenHash, userId, now).first();
    return tokenRecord || null;
  }
  async revokeRefreshToken(tokenId) {
    const now = Math.floor(Date.now() / 1e3);
    await this.db.prepare("UPDATE refresh_tokens SET revoked_at = ? WHERE id = ?").bind(now, tokenId).run();
  }
  async revokeAllUserRefreshTokens(userId) {
    const now = Math.floor(Date.now() / 1e3);
    await this.db.prepare("UPDATE refresh_tokens SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL").bind(now, userId).run();
  }
  async cleanupExpiredTokens() {
    const now = Math.floor(Date.now() / 1e3);
    await this.db.prepare("DELETE FROM verification_tokens WHERE expires_at < ?").bind(now).run();
    await this.db.prepare("DELETE FROM refresh_tokens WHERE expires_at < ?").bind(now).run();
  }
};

// src/routes/auth.ts
async function handleInitiate(c, authService, phone_number) {
...
__name(handleInitiate, "handleInitiate");
async function handleRefreshToken(c, authService) {
...
async function handleLogout(c, authService) {
...
async function handleValidate(c, authService) {
...
function handleWebhookVerification(c) {
...
// src/routes/user.ts
async function handleGetUserMe(c, userService) {
...
__name(handleGetUserMe, "handleGetUserMe");
async function handlePutUserMe(c, userService) {
  const authInfo = c.get("authInfo");
  if (!authInfo) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const { name } = await c.req.json();
  await c.env.DB.prepare("UPDATE users SET name = ? WHERE id = ?").bind(name, authInfo.userId).run();
  return c.json({ success: true });
}
__name(handlePutUserMe, "handlePutUserMe");
...
// src/do/WebhookProcessorDO.ts
var WebhookProcessorDO = class {
  static {
    __name(this, "WebhookProcessorDO");
  }
        ...
        for (const change of entry.changes) {
          if (change.field === "messages") {
            ...
            if (value.messages && value.messages.length > 0) {
              ...
              for (const message2 of value.messages) {
                const from = message2.from;
                const normalizedFrom = normalizePhoneNumber(from);
                console.log(`[WebhookProcessor] Processing message from ${from} (normalized: ${normalizedFrom}), type: ${message2.type}`);
                if (message2.type === "text" && message2.text) {
                  const text = message2.text.body;
                ...
                if (message2.type === "interactive" && message2.interactive?.type === "button_reply") {
                  const buttonPayload = message2.interactive.button_reply.id;
  ...
  async handleAuthButtonPayload(authService, whatsappService, phoneNumber, token) {
    ...
    if (result) {
      ...
      const loginTokenPayload = await authService.verifyLoginTokenOnly(token);
...
// src/do/AuthSessionDO.ts
var AuthSessionDO = class {
  static {
    __name(this, "AuthSessionDO");
  }
  constructor(state, env) {
    this.state = state;
    this.env = env;
  ...
  async fetch(request) {
    ...
    switch (url.pathname) {
      ...
      case "/send-tokens":
        ...
        const { authToken, refreshToken, userId } = await request.json();
  ...
  sendTokensToClient(authToken, refreshToken, userId) {
    const message2 = JSON.stringify({ event: "auth_success", authToken, refreshToken, userId });
...
var phoneSchema = external_exports.object({
  phone_number: external_exports.string().min(10).max(15)
});
var tokenSchema = external_exports.object({
  token: external_exports.string().min(10)
});
function createJsonValidator(schema) {
  return zValidator("json", schema);
}
__name(createJsonValidator, "createJsonValidator");
var validateToken = createJsonValidator(tokenSchema);
...
app.post("/api/webhook", async (c) => {
  ...
  const newRequest = new Request(c.req.url, {
    ...
    body: c.req.raw.body ? await c.req.raw.arrayBuffer() : null,
    // Read body as arrayBuffer
    redirect: c.req.raw.redirect,
    cf: c.req.raw.cf
    // Preserve cf properties
  });
...
// .wrangler/tmp/bundle-PxxfFi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PxxfFi/middleware-loader.entry.ts
